...
class HashTab(object):
    def __init__(self, size):
        ...
        self.rand = random.randint(0, int(self.__size))    

   # return current number of keys in table    
    def __len__(self): return self.__numRecords

    def hashFunc(self, s, rand):
        x = Utils.hash_func_int(str(s))          # hash twice 
        y = Utils.hash_func_int(str(s) + str(x))
    ...
    def insert(self, k, d):
        ...
        position1, position2 = self.hashFunc(n.key, self.rand)  # hash
        ...
        for i in range(5):
            ...
            if pos == position1:                            # if we're checking the 1st table right now, 
                position1, position2 = self.hashFunc(n.key, self.rand) # hash the displaced node,
                pos = position2                             # and check its 2nd position 
                table = self.__hashArray2                   # in the 2nd table (next time through loop)
            else:                               
                position1, position2 = self.hashFunc(n.key, self.rand) # otherwise, hash the displaced node,
    ...
    def rehash(self, size):
        ...
        #print("rehashing with", self.rand)
        #self.rand = random.randint(0, int(math.pow(2, 32)))      
    ...
    def find(self, k):
        pos1, pos2 = self.hashFunc(k, self.rand)               # check both positions the key/data
    ...
    def delete(self, k):
        pos1, pos2 = self.hashFunc(k, self.rand)  
